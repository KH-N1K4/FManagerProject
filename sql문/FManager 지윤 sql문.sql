...
SELECT CHAT_ROOM_NO
         ...
         ,TO_CHAR(NVL((SELECT MAX(CHAT_SEND_TIME) CHAT_SEND_TIME 
               FROM CHAT M
               JOIN CHAT_ROOM ON CHAT_ROOM.CHAT_ROOM_NO = M.CHAT_ROOM_NO
               WHERE R.CHAT_ROOM_NO  = M.CHAT_ROOM_NO
               AND ((CHAT_ROOM_OPENMEM_UPDATE_DATE IS NULL OR CHAT_SEND_TIME >= CHAT_ROOM_OPENMEM_UPDATE_DATE) AND CHAT_OPENMEM_NO = 30)
               AND ((CHAT_ROOM_CLIENT_UPDATE_DATE IS NULL OR CHAT_SEND_TIME >= CHAT_ROOM_CLIENT_UPDATE_DATE) AND CLIENT_NO = 30)), CHAT_ROOM_CREATE_DATE), 
               'YYYY.MM.DD hh:mm') SEND_TIME
         ...
         ,(SELECT COUNT(*) FROM CHAT M WHERE M.CHAT_ROOM_NO = R.CHAT_ROOM_NO AND READ_FL = 'N' AND SENDER_NO != 30) NOT_READ_COUNT
         ,(SELECT MAX(CHAT_NO) CHAT_SEND_TIME FROM CHAT M WHERE R.CHAT_ROOM_NO  = M.CHAT_ROOM_NO) MAX_MESSAGE_NO
      FROM CHAT_ROOM R
