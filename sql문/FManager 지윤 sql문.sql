...
ALTER SEQUENCE SEQ_CHAT_ROOM_NO INCREMENT BY 1;
...
SELECT (SELECT TO_CHAR(SUM(PAYMENT_PRICE), 'FM999,999,999,999')  --,PROFIT_TYPE 
FROM (SELECT  p.*,TO_CHAR(PAYMENT_DATE,'YYYY-MM') PAYMENT_DATE_STRING 
	FROM (SELECT * FROM SETTLEMENT
			JOIN TRADE USING(TRADE_NO)
			JOIN SERVICE USING(SERVICE_NO)
...
SELECT  p.*,TO_CHAR(PAYMENT_DATE,'YYYY-MM-DD hh:mm') PAYMENT_DATE_STRING,
...
SELECT CHAT_ROOM_NO
         ...
         ,TO_CHAR((SELECT 
				CASE WHEN (CHAT_OPENMEM_NO = 30 AND CHAT_ROOM_OPENMEM_UPDATE_DATE IS NULL) THEN NVL(SEND_TIME,CHAT_ROOM_CREATE_DATE)
					 WHEN (CHAT_OPENMEM_NO = 30 AND CHAT_ROOM_OPENMEM_UPDATE_DATE IS NOT NULL AND SEND_TIME > CHAT_ROOM_OPENMEM_UPDATE_DATE) THEN NVL(SEND_TIME,CHAT_ROOM_CREATE_DATE)
					 ...
					 WHEN (CLIENT_NO = 30 AND CHAT_ROOM_CLIENT_UPDATE_DATE IS NOT NULL AND SEND_TIME > CHAT_ROOM_CREATE_DATE) THEN NVL(SEND_TIME,CHAT_ROOM_CREATE_DATE)
         ...
         ,(SELECT COUNT(*) FROM (SELECT * FROM CHAT JOIN CHAT_ROOM USING(CHAT_ROOM_NO)) M 
         	...
         	AND READ_FL = 'N' AND SENDER_NO != 30
         	AND (CHAT_ROOM_OPENMEM_UPDATE_DATE IS NULL OR CHAT_SEND_TIME >= CHAT_ROOM_OPENMEM_UPDATE_DATE AND CHAT_OPENMEM_NO = 30)
            AND (CHAT_ROOM_CLIENT_UPDATE_DATE IS NULL OR CHAT_SEND_TIME >= CHAT_ROOM_CLIENT_UPDATE_DATE AND CLIENT_NO = 30)) NOT_READ_COUNT
         ...
         NVL2((SELECT CHAT_OPENMEM_NO FROM CHAT_ROOM R2
            WHERE R2.CHAT_ROOM_NO = R.CHAT_ROOM_NO
            AND R2.CHAT_OPENMEM_NO = 30),
            (SELECT MEMBER_NICKNAME FROM MEMBER LEFT JOIN FREELANCER USING(MEMBER_NO) WHERE MEMBER_NO = R.CLIENT_NO),
            (SELECT MEMBER_NICKNAME FROM MEMBER LEFT JOIN FREELANCER USING(MEMBER_NO) WHERE MEMBER_NO = R.CHAT_OPENMEM_NO)
            ) CLIENT_NICKNAME 
      FROM CHAT_ROOM R
      WHERE (CHAT_OPENMEM_NO = 30
      OR CLIENT_NO = 30)
      AND ((CHAT_ROOM_OPENMEM_DEL_FL = 'N' AND CHAT_OPENMEM_NO = 30)
	  OR (CHAT_ROOM_CLIENT_DEL_FL = 'N' AND CLIENT_NO = 30))
