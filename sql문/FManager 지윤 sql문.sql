...
------------------------------------------------------------------------------------------------
--거래별 서비스 테이블이랑 조인해서 선택해오기 --- 작업물 발송 횟수 들고오기  --최종
--1:진행 중, 2: 작업 완료
SELECT t.*,
	CASE WHEN FREELANCER_DONE_FL = 1 THEN '진행 중'
	     WHEN FREELANCER_DONE_FL = 2 THEN '작업 완료'
...
FROM (SELECT * FROM TRADE
	...
	LEFT JOIN TRADE_REPORT USING(TRADE_NO) HAVING REPORT_PERSON = FREELANCER_NO) t
WHERE FREELANCER_NO = 30
ORDER BY TRADE_NO;
AND MAIN_CATEGORY_NO = 2 
AND FREELANCER_DONE_FL = 1
...
VALUES(SEQ_TRADE_REPORT_NO.NEXTVAL,#{신고자},#{피신고자},#{내용},DEFAULT,#{파일경로},#{거래번호});
INSERT INTO "TRADE_REPORT" 
(TRADE_REPORT_NO, REPORT_PERSON, REPORTED_PERSON,TRADE_REPORT_CONTENT,REFUND_FL,TRADE_NO) 
VALUES 
(SEQ_TRADE_REPORT_NO.NEXTVAL,#{신고자},#{피신고자},#{내용},DEFAULT,#{파일경로},#{거래번호})


--1초에 한번씩 요구사항이 변경되면서 전화를 하루에 50번 이상 오셔서 거래 안하고 싶어요
--돈 하나도 안 받아도 상관 없으니까 제발 거래 취소해주세요!

DELETE FROM "TRADE_REPORT" 
WHERE TRADE_REPORT_NO=5;
--SEQ_WORK_NO--SEQ_TRADE_REPORT_NO
SELECT SEQ_TRADE_REPORT_NO.NEXTVAL FROM DUAL;--시퀀스 되돌리기기
ALTER SEQUENCE SEQ_TRADE_REPORT_NO INCREMENT BY -2;
ALTER SEQUENCE SEQ_TRADE_REPORT_NO INCREMENT BY 1;
...
INSERT INTO "SETTLEMENT"
VALUES(SEQ_SETTLEMENT_NO.NEXTVAL,2,30,2,SYSDATE,100000,2);
...
--------------------------------------------------------
SELECT SUM(PAYMENT_PRICE),PAYMENT_DATE_STRING 
