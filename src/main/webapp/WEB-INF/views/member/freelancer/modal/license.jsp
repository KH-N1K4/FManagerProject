<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>

<div class="modal_body">
        <span class="modal_close3">x</span>
        <div id="licenseInputMain">
                <div>
                    <div class="item">자격증명</div>
                    <div>
                        <input type="text" name="licenseContent" id="licenseContent1" class="input3">
                    </div>
                </div>
                <div>
                    <div class="item">발급일</div>
                    <div>
                        <input type="text" name="licenseContent" id="licenseContent2" class="input3">
                    </div>
                </div>
                <div>
                    <div class="item">발급 기관</div>
                    <div>
                        <input type="text" name="licenseContent" id="licenseContent3" class="input3">
                    </div>
                </div>
                <div>
                    <button id="sendContent3">적용</button>
                </div>
        </div><%-- licenseInputMain --%>
    </div>

    <%-- <script src="/resources/js/member/freelancer/enrollFreelancerPopup.js"></script> --%>
<script>


</script>
